...
$Help_ID = $_GET['updateid']; // Use the correct variable name for Help_ID
...
if (isset($_POST['submit'])) {
    ...
    $sql = "UPDATE Help
            SET Status = '$status'
            WHERE Help_ID = '$Help_ID'"; // Make sure you're using the correct Help_ID variable
    ...
    if (mysqli_query($conn, $sql)) {
        header("Location: Customersupport-helpcentre.php"); // Redirect to the dashboard
...
<form action="" method="post">
    ...
    <select name="status" style="width:100px; height:30px; border:none; border-radius:10px;" id="Solved_<?php echo $row["Help_ID"]; ?>">
        <option value="Active" <?php if ($row["Status"] == 'Active') echo 'selected'; ?>>Active</option>
        <option value="Closed" <?php if ($row["Status"] == 'Closed') echo 'selected'; ?>>Closed</option>
    </select>
    <br><br>
    <input type="submit" name="submit" value="Update Status">
    <input type="button" value="Cancel" onclick="window.location.href='Customersupport-helpcentre.php';">
</form>
